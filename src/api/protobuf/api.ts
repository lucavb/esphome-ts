...
export const SubscribeHomeassistantServicesRequest = {
  ...
  decode(reader: Reader, length?: number): SubscribeHomeassistantServicesRequest {
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = Object.create(baseSubscribeHomeassistantServicesRequest) as SubscribeHomeassistantServicesRequest;
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        default:
...
export const ListEntitiesClimateResponse = {
  ...
  fromJSON(object: any): ListEntitiesClimateResponse {
    ...
    if (object.supportsTwoPointTargetTemperature !== undefined && object.supportsTwoPointTargetTemperature !== null) {
